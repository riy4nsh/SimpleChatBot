...
from utils.text_utils import (
    clean_text,
    hinglish_normalize,
    simple_spell_corrector_build,
    simple_spell_correct,
)
...
# CORS configuration
# ----------------------
FRONTEND_URL = "https://simplechatbot-1.onrender.com"
LOCAL_ORIGINS = ["http://localhost:3000", "http://127.0.0.1:3000"]

app.add_middleware(
    CORSMiddleware,
    allow_origins=[*LOCAL_ORIGINS, FRONTEND_URL],
...
if not os.path.exists(RESP_PATH):
    raise FileNotFoundError(f"responses.json not found at {RESP_PATH}")
...
if SBERT_AVAILABLE:
    try:
        ...
        print("[WARN] SBERT init failed, falling back to TF-IDF. Error:", e)
        use_sbert = False
...
def compute_similarity(query: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    if use_sbert and model is not None and Q_EMB is not None:
...
@app.get("/health")
def health():
    return {"status": "ok", "use_sbert": use_sbert}
...
def reply(payload: Query):
    ...
    try:
        ...
        best_score = float(top_scores[0]) if len(top_scores) > 0 else 0.0
        ...
        return {
            "answer": "I’m not fully sure about that. Could you rephrase the question?",
            "score": best_score,
        }

    except Exception as e:
        # log the exception and return a friendly message
        print("[ERROR] Exception in /reply:", str(e))
        raise HTTPException(status_code=500, detail="Server error processing the request.")
